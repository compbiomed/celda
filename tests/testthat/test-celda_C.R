...
load("tests/celdaCsim.rda")
load("tests/celdaC.rda")

model_C = getModel(celdaC.res, K = 5)[[1]]
factorized_C <- factorizeMatrix.celda_C(celda.mod = model_C, counts = celdaC.sim$counts)
counts.matrix_C <- celdaC.sim$counts
...
test_that(desc = "Checking factorize matrix, counts vs proportions",{
  expect_equal(TRUE,all(factorized_C$counts$sample.states/sum(factorized_C$counts$sample.states)
                        == factorized_C$proportions$sample.states))
})

#Checking dimension of factorize matrix
test_that(desc = "Checking factorize matrix dimension size",{
  expect_equal(5, nrow(factorized_C$proportions$sample.states))
})
...
test_that(desc = "calculateLoglikFromVariables.celda_C returns correct output for various params", {
  expect_lt(calculateLoglikFromVariables(z = celdaC.sim$z, 
                                            ...
                                            counts=celdaC.sim$counts), 0)
})


#normalizeCounts
test_that(desc = "Checking normalizeCounts doesn't change dimensions",{
  norm.counts <- normalizeCounts(counts.matrix_C)
  expect_equal(dim(norm.counts),dim(counts.matrix_C))
  expect_equal(rownames(norm.counts),rownames(counts.matrix_C))
  expect_equal(colnames(norm.counts),colnames(counts.matrix_C))
})
...
test_that(desc = "Checking diffExp",{
 expect_equal(class(diffexp_K1 <- diffExp(counts = counts.matrix_C, celda.mod = model_C, c1 = 1)),
...
test_that(desc = "Checking celda_C to see if it runs without errors",{
  ...
  celdaC.res <- celda_C(counts = celdaC.sim$counts, K = 5, max.iter = 15, random.state.order = FALSE)
  expect_true(class(celdaC.res) == "celda_C")
