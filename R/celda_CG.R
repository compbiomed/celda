...
celda_CG = function(counts, sample.label=NULL, K, L,
                    ...
                    z.init = NULL, y.init = NULL, random.state.order=TRUE, 
                    logfile=NULL) { 
  ...
  while(iter <= max.iter & num.iter.without.improvement <= stop.iter) {
	...
	next.z = do.call(algorithm.fun, list(counts=n.TS.by.C, m.CP.by.S=m.CP.by.S, n.G.by.CP=n.TS.by.CP, n.CP=n.CP, n.by.C=n.by.C, z=z, s=s, K=K, nG=L, nM=nM, alpha=alpha, beta=beta, random.state.order=random.state.order))
...
cCG.decomposeCounts = function(counts, s, z, y, K, L) {
  ...
  if(cCG.global_yChanged){
    ...
    cCG.global_n.by.TS <<- as.integer(rowSumByGroup(matrix(n.by.G,ncol=1), y=y, L=L))
  }
  ...
  if(cCG.global_zChanged){
    cCG.global_n.CP.by.G <<- colSumByGroup(counts, group=z, K=K)
  }
  n.CP.by.G = cCG.global_n.CP.by.G
  
  if(cCG.global_zChanged || cCG.global_yChanged){
    cCG.global_n.CP.by.TS <<- t(colSumByGroup(n.TS.by.C, group=z, K=K))
