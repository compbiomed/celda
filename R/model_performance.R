...
#' Calculate and visualize perplexity of all models in a celda_list, with k-fold cross validation
...
#' @param folds Numvber of folds for cross validation
#' @param title Title to be appended to the perplexity plot title. Default is "".
#' @return A list with a data frame summarizing all of the calculated perplexities
#' @export
calculatePerplexityWithCrossValidation = function(celda.list, counts, folds, title="") {
  if (celda.list$content.type != "celda_CG")  {
    stop("This function is just for testing, and only accepts celda_CG Models.")
  }
  
  x.validation.res = lapply(celda.list$res.list,
                            function(mod) {
                              split.idx = sample(1:folds, ncol(counts), replace=TRUE)
                              message(paste("======================================\n", 
                                            "K:", mod$K, "L:", mod$L))
  ...
  all.x.validation.res = do.call("rbind", x.validation.res)
  return(all.x.validation.res)
}
